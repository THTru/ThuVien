...
<div class="wrapper" v-if="readerDetail.id!=0">
    <NavBar/>
    <div>
        Mã bạn đọc: {{ readerDetail.id }}<br>
        Tên: {{ readerDetail.tenbandoc }}
    </div>
    ...
    <div v-for="item in readerHistory" v-bind:class="{expired: checkdate(item.phieumuon.ngayhentra, item.ngaytra)}">
        ...
        <div class="list-cell">{{ item.phieumuon.ngaymuon }}</div>
        <div class="list-cell">{{ item.phieumuon.ngayhentra }}</div>
        <div class="list-cell" v-if="item.ngaytra=='1900-01-01'">_</div>
        <div class="list-cell" v-else>{{ item.ngaytra }}</div>
