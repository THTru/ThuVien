...
return new class extends Migration
{
    ...
    public function up()
    {
        Schema::create('chitietmuon', function (Blueprint $table) {
            $table->id();
            $table->unsignedBigInteger('id_phieumuon');
            $table->unsignedBigInteger('id_sach');
            $table->date('ngaytra')->default('1900-1-1');
            $table->string('trangthaitra')->default('Chưa trả');
            $table->timestamps();

            $table->foreign('id_sach')->references('id')->on('sach')->constrained()->onUpdate('cascade')->onDelete('cascade');
            $table->foreign('id_phieumuon')->references('id')->on('phieumuon')->constrained()->onUpdate('cascade')->onDelete('cascade');
        });
